  ...
  const {data} =  await axios.post('https://ecommerce.routemisr.com/api/v1/cart',pId,{headers})
    ...
    Swal.fire({
      ...
      return<>
        ...
        {/* py-1 md:py-2 px-32 sm:px-24 md:px-8 lg:px-14 xl:px-24  */}

        {/* heart and add to cart */}
        <div className="absolute bottom-3 flex gap-3 justify-between overflow-hidden">
        <button onClick={()=>{addToCart(p.id)}} className="bg-green-600 rounded-md py-1 md:py-2 px-3 sm:px-16 md:px-24 text-center self-start translate-y-52 group-hover:translate-y-0 transition duration-200 ease-in-out">+ add</button>
        <FontAwesomeIcon  onClick={()=>{addWishList(p.id)}} className = {`text-lg sm:text-xl md:text-lg lg:text-2xl self-end ${heart[p.id] && 'text-red-700'} `} icon={faHeart}/>
        </div>
