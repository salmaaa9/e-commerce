...
function Navbar() {
  const [cartCount,setartCount] = useState(0)
  ...
  return (
  <>
    <nav className="bg-slate-50 dark:bg-gray-900 fixed-top w-full z-20 start-0 border-gray-200 dark:border-gray-600">
    ...
    <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
      {/* cart */}
      <Link to="home" className="flex items-center space-x-3 rtl:space-x-reverse relative me-2">
      <button onClick={()=>{setartCount(cartCount+1)}} className="cart-count bg-green-800 px-2 absolute text-white bottom-5 start-7 rounded-lg">{cartCount}</button>
      <FontAwesomeIcon className="dark:text-white text-2xl" icon={faCartShopping} />
      </Link>

      <button type="button" className="dark:text-white focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-4 py-2 text-center">Log out</button>
      {/* dark toggler */}
      <button onClick={()=>{setDark(!dark)}} id="theme-toggle" type="button" className="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5">
        {dark?
    ...
    <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
      <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-slate-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-transparent dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
        ...
        <li>
          <Link to="products" className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-green-700 md:p-0 md:dark:hover:text-green-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">Products</Link>
        </li>
        <li>
          <Link to="categories" className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-green-700 md:p-0 md:dark:hover:text-green-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">Categories</Link>
        </li>
        <li>
          <Link to="brands" className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-green-700 md:p-0 md:dark:hover:text-green-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">Brands</Link>
        </li>
      </ul>
    </div>
